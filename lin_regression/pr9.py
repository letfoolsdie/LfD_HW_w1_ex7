...
w_average = [0., 0., 0., 0., 0., 0.]
for i in range(1000):

    data = np.array(builddataset(1000))
...
#print(total_error/1000)
    ...
    outtargetv = np.array(evaluatedatatargetfunction(outofsample))
    outvectors = np.array(transformdata(outofsample))
    hypov = evaluatehypofunction(w, outvectors)
    out_sample_error = classificationerror(hypov, outtargetv)
    total_out_error += out_sample_error
print(total_out_error/1000)